...
| **Emoji Text Renderer** | Renders Unicode emoji in page text and flags in text fields and editors using a flag-only font range, preserving ordinary characters. | 1.0.2 | [![Install](https://img.shields.io/badge/Install-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/browser-tools/emoji-text-renderer/Emoji_Text_Renderer.user.js) |
| **RuTracker Digital Release Linker** | Links exact Deezer release pages by barcode/catalog number, handles nested RuTracker spoilers correctly, shows an expanded list of releases that were not found, preserves existing concrete release URLs while treating artist pages as placeholders, recognizes identifiers before trailing `(by ...)` annotations, catalog numbers containing spaces such as `[SUB 003]` and uppercase letter-only catalog numbers such as `[BLCKTNLRMXCOMP]`, and catalog/reissue-year forms such as `[RBR0869DL - 2021]`, replaces placeholder WEB links with the resolved Deezer source, accepts source lines with or without `[hr]`, uses Deezer/Beatport links only from the exact matched MusicBrainz release, including Deezer track relationships on its recordings, processes unresolved `WEB|redacted.sh` / `WEB|redacted.ch` entries, falls back to MusicBrainz-linked Beatport URLs without scraping Beatport, and adds country flag emoji, including proper England, Scotland, and Wales subdivision flags. | 1.1.18 | [![Install](https://img.shields.io/badge/Install-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/26ac40154d516f2e7d85da9b4ded5f6541d71af2/browser-tools/rutracker-digital-release-linker/RuTracker_Digital_Release_Linker.user.js) |

## MusicBrainz Tools

| Project | Description | Version | Download |
| --- | --- | ---: | --- |
| **MusicBrainz - Safe Recording Matcher** | Matches by title and artist, or by pasted ISRC list/table in release track order. ISRC matches choose the uniquely closest eligible length; both modes require matching title, full artist credit and a length difference of at most 7 seconds. | 1.1.0 (not live tested) | [![Install](https://img.shields.io/badge/Install-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/musicbrainz-tools/safe-recording-matcher/MusicBrainz_Safe_Recording_Matcher.user.js) |
...
| **MusicBrainz - Auto-Select Single Disc ID Artist** | Automatically continues the Disc ID attachment workflow when the artist search returns exactly one result; leaves 0 or multiple results for manual selection. | 1.0.0 | [![Install](https://img.shields.io/badge/Install-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/musicbrainz-tools/disc-id-auto-select-artist/MusicBrainz_Auto_Select_Single_Disc_ID_Artist.user.js) |
...
| **MusicBrainz - Harmony Importer** | Runs a Harmony-style importer directly on MusicBrainz. Initial Apple Music provider supports album URL/barcode lookup, track and ISRC preview, barcode duplicate checks, one-click ISRC copying, and Add Release seeding without using the Harmony website. | 0.1.0 (not live tested) | [![Install](https://img.shields.io/badge/Install-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/musicbrainz-tools/harmony-importer/MusicBrainz_Harmony_Importer.user.js) |
| **Harmony - Copy ISRCs** | Adds a one-click Copy ISRCs button to Harmony release tracklists and copies all displayed ISRCs in track order. | 1.0.2 | [![Install](https://img.shields.io/badge/Install-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/musicbrainz-tools/harmony-copy-isrcs/Harmony_Copy_ISRCs.user.js) |
