...
| **RuTracker Digital Release Linker** | Links exact Deezer release pages by barcode/catalog number, processes unresolved `WEB|redacted.sh` / `WEB|redacted.ch` entries, uses MusicBrainz-linked Beatport release URLs as a fallback without scraping Beatport, repairs generic Deezer artist links when possible, and adds the matching flag emoji after the RuTracker country field. | 1.1.1 | [![Install](https://img.shields.io/badge/Install-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/browser-tools/rutracker-digital-release-linker/RuTracker_Digital_Release_Linker.user.js) |
