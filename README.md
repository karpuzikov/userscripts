# Userscripts & Tools

A collection of my Tampermonkey userscripts, apps, scripts, and utilities.

## Instagram Tools

| Project | Description | Version | Download |
| --- | --- | ---: | --- |
| **Instagram Force Full Quality Media** | Forces the highest-quality Instagram post/Reel media while leaving profile, search, and Explore thumbnails untouched. | 4.0.0 | [![Download](https://img.shields.io/badge/Download-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/instagram-full-quality/Instagram_Force_Full_Quality_Media.user.js) |

## MusicBrainz Tools

| Project | Description | Version | Download |
| --- | --- | ---: | --- |
| **MusicBrainz BOIU Cover Art Removal** | Adds a **BOIU** button next to Edit/Remove for each cover image. One click removes that image with the edit note `better one is uploaded`. | 1.0.0 | [![Download](https://img.shields.io/badge/Download-.user.js-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/musicbrainz-tools/MusicBrainz_BOIU_Cover_Art_Removal.user.js) |

## Media Tools

| Project | Description | Version | Download |
| --- | --- | ---: | --- |
| **Archive Media Compressor** | Recursively compresses and standardizes video/photo archives, preserves folder structure, auto-detects hardware encoders, and installs required tools on Windows. | 1.0.0 | [![Download](https://img.shields.io/badge/Download-.zip-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/karpuzikov/userscripts/main/archive-media-compressor/ArchiveMedia.zip) |

## Notes

- Tampermonkey userscripts use direct `.user.js` download links so clicking **Download** should open the userscript installer when Tampermonkey is installed.
- Scripts, apps, and tools are grouped by category above.
- Source files remain available in project folders or downloads.

## License

MIT
